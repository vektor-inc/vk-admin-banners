...
|  admin_url  | true の場合管理画面の url に変換されます。<br>その際は link_url には URL の ```/wp-admin/``` までを削除した ```theme-install.php?search=lightning``` のような部分を入力する必要があります。 |
...
|  alt  | ALT 属性のテキスト |
|  language  | 言語 ( 現状 ja or en )  |
|  admin_url  | true の場合管理画面の url に変換されます。<br>その際は link_url には URL の ```/wp-admin/``` までを削除した ```plugin-install.php?s=vk+all+in+one+expansion+unit&tab=search&type=term``` のような部分を入力する必要があります。 |
